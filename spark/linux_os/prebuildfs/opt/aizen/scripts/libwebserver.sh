...
#
# AIZEN web server handler library
...
web_server_execute() {
    ...
    (
        . "/opt/aizen/scripts/lib${web_server}.sh"
        . "/opt/aizen/scripts/${web_server}-env.sh"
        "$@"
    )
}
...
web_server_list() {
    ...
    for web_server in "${supported_web_servers[@]}"; do
        [[ -f "/opt/aizen/scripts/${web_server}-env.sh" ]] && existing_web_servers+=("$web_server")
    done
...
web_server_validate() {
    ...
    if [[ -z "$(web_server_type)" || ! " ${supported_web_servers[*]} " == *" $(web_server_type) "* ]]; then
        ...
        print_validation_error "Could not load the $(web_server_type) web server library from /opt/aizen/scripts. Check that it exists and is readable."
    fi
...
web_server_start() {
    ...
    if [[ "${AIZEN_SERVICE_MANAGER:-}" = "systemd" ]]; then
        systemctl start "aizen.$(web_server_type).service"
    else
        "${AIZEN_ROOT_DIR}/scripts/$(web_server_type)/start.sh"
    fi
}
...
web_server_stop() {
    ...
    if [[ "${AIZEN_SERVICE_MANAGER:-}" = "systemd" ]]; then
        systemctl stop "aizen.$(web_server_type).service"
    else
        "${AIZEN_ROOT_DIR}/scripts/$(web_server_type)/stop.sh"
    fi
}
...
web_server_restart() {
    ...
    if [[ "${AIZEN_SERVICE_MANAGER:-}" = "systemd" ]]; then
        systemctl restart "aizen.$(web_server_type).service"
    else
        "${AIZEN_ROOT_DIR}/scripts/$(web_server_type)/restart.sh"
    fi
}
...
web_server_reload() {
    if [[ "${AIZEN_SERVICE_MANAGER:-}" = "systemd" ]]; then
        systemctl reload "aizen.$(web_server_type).service"
    else
        "${AIZEN_ROOT_DIR}/scripts/$(web_server_type)/reload.sh"
    fi
}
