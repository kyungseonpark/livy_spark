...
. /opt/aizen/scripts/libvalidations.sh
...
generate_systemd_conf() {
    ...
    local -r service_file="${systemd_units_dir}/aizen.${service_name}.service"
    # Default values
...
[Unit]
Description=AIZEN service for ${name}
# Starting/stopping the main aizen service should cause the same effect for this service
PartOf=aizen.service
...
[Install]
# Enabling/disabling the main aizen service should cause the same effect for this service
WantedBy=aizen.service
EOF
}
